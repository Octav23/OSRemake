...
public class CloseInterfacePacketHandler implements PacketHandler {
	...
	public void handle(Player player, Packet packet) {
		if(packet.getOpcode() == 64) {
			...
			player.getActionSender().sendMessage("InterfaceId - ChildId : " + interfaceId + " - " + childId);
			switch(interfaceId) {
		/*
		 * Hard leather body interface.
		 */
		case 309:
			if(Crafting.hardLeatherBodies(player, childId)) {
				return;
			}
			break;
		case 51:
			Cooking.handleInterfaceButtons(player, childId);
			break;
			case 7:
			case 64:
			case 65:
			...
			case 53:
				...
				break;
			case 47:
			if(player.getInterfaceAttribute("craftingnew_log") != null) {
					int logIndex = -1;
					int productionCount = -1;
					switch(childId) {
					case 7:
					...
					case 8:
						...
						player.setInterfaceAttribute("craftingnew_index", logIndex);
						break;
					}
					if(productionCount != -1) {
						player.getActionQueue().addAction(new CraftingNew(player, productionCount, logIndex, (Log1) player.getInterfaceAttribute("fletching_log")));
						player.getActionSender().removeChatboxInterface();
					}
				}
			...
			if(player.getInterfaceAttribute("craftingnew_log") != null) {
					...
					if(productionCount != -1) {
						player.getActionQueue().addAction(new CraftingNew(player, productionCount, logIndex, (Log1) player.getInterfaceAttribute("craftingnew_log")));
						player.getActionSender().removeChatboxInterface();
					}
				}
				if(player.getInterfaceAttribute("fletching_log") != null) {
