...
import org.rs2server.rs2.content.misc.ExplosivePotion;
import org.rs2server.rs2.content.misc.SpadeDig;
...
import org.rs2server.rs2.model.skills.CraftingNew;
import org.rs2server.rs2.model.skills.CraftingNew.Log1;
...
public class ItemOptionPacketHandler implements PacketHandler {
	...
	public void handle(Player player, Packet packet) {
		...
		case OPTION_2_2:
			handleItemOption2_2(player, packet);
			break;
	...
	private void handleItemOptionDrop(Player player, Packet packet) {
		...
		if(player.getSkills().getTotalLevel() <= 150){
			player.getActionSender().sendMessage("You need a total level of 150 to drop items.");
			return;
		}
	...
	/**
	 * Handles item option 2_2.
	 * @param player The player.
	 * @param packet The packet.
	 */
	private void handleItemOption2_2(Player player, Packet packet) {		
		int id = packet.getShortA() & 0xFFFF;
		int interfaceSet = packet.getInt();
		int slot  = interfaceSet >> 16;
		int unknown = packet.getLEShort() & 0xFFFF;
		
		if(player.getCombatState().isDead()) {
			return;
		}
		WaterFillables.checkForItemEmpty(player, id, slot);
		logger.info("Unhandled item option 2_2 : " + id + " - " + unknown + " - " + slot + ".");
		Item item = null;
		...
		case Inventory.INTERFACE:
			if(slot >= 0 && slot < Inventory.SIZE) {
				...
				if(herb != null) {
					...
					player.getSkills().addExperience(Skills.HERBLORE, 15);
					return;
				}
	...
	private void handleItemOnItem(final Player player, Packet packet) {
		...
		case Inventory.INTERFACE:
			if(slot >= 0 && slot < Inventory.SIZE) {
				...
				if(usedItem.getId() == 11286 && withItem.getId() == 1540) {
					if(usedItem == null || withItem == null) {
						return;
					}
						player.getInventory().remove(new Item(11286));
						player.getInventory().remove(new Item(1540));
						player.getActionSender().sendMessage("You attach the shield to the visage.");
				...
				if(Crafting.handleItemOnItem(player, usedItem.getId(), withItem.getId())) {
				return;
				}
						//if (usedItem.getId() == 1733 && withItem.getId() == 1743 || withItem.getId() == 1733 && usedItem.getId() == 1743) {
				...
				if(usedItem.getId() == 1733 || withItem.getId() == 1733) {
					...
					Log1 log = Log1.forId(logItem.getId());
					if(log != null) {
						interfaceId = 309;
						if(log.getItem().length > 1) {
							interfaceId = 301 + log.getItem().length;
						...
						player.getActionSender().sendChatboxInterface(interfaceId);
						player.setInterfaceAttribute("craftingnew_log", log);
						return;
					}
				}
				if(Cooking.checkForItemOnItem(player, usedItem, withItem, slot, slot)) {
				return;
				}
