...
public class Barrows {
	...
	public void completedGame() {
		...
		case 0:
		player.getInventory().add(new Item((Misc.random(ahrims[0], ahrims[3]))));
		break;
		case 1:
		player.getInventory().add(new Item((Misc.random(dharoks[0], dharoks[3]))));
		break;
		case 2:
		player.getInventory().add(new Item((Misc.random(guthans[0], guthans[3]))));
		break;
		case 3:
		player.getInventory().add(new Item((Misc.random(karils[0], karils[3]))));
		break;
		case 4:
		player.getInventory().add(new Item((Misc.random(torags[0], torags[3]))));
		break;
		case 5:
		player.getInventory().add(new Item((Misc.random(veracs[0], veracs[3]))));
		break;
	}
}
	...
	public void handleObjectOption(final int id) {
		if(id == AHRIM_S && !player.AHRIMS) {
		player.AHRIMS = true;
			spawn(2025);
			return;
		}
		if(id == DHAROK_S && !player.DHAROKS) {
			spawn(2026);
			player.DHAROKS = true;
			return;
		}
		if(id == GUTHAN_S && !player.GUTHANS) {
			spawn(2027);
						player.GUTHANS = true;
			return;
		}
		if(id == KARIL_S && !player.KARILS) {
			spawn(2028);
						player.KARILS = true;
			return;
		}
		if(id == TORAG_S && !player.TORAGS) {
			spawn(2029);
						player.TORAGS = true;
			return;
		}
		if(id == VERAC_S && !player.VERACS) {
			spawn(2030);
						player.VERACS = true;
			return;
		}
