...
public class Cooking {
	...
	/**
	 * An array holding all the objects we can cook on.
	 * Eg: Range, Stove, Fire etc.
	 * Horrible conventions xD
	 */
	private static final int[][] COOKING_OBJECTS = { 
		{
			114/* Cooking range */,
			2728/* Range */, 2729/* Range */, 2730/* Range */,
			2731/* Range */,
			2859/* Range */,
			3039/* Range (in Falador Castle upstairs I guess) */,
			4172/* Cooking range. */,
			5275/* Cooking range. */, 
			8750/* Cooking range. */, 
			9682/* Range */,
			...
			9087/* Stove */, 
			9088/* Stove */, 
			12269/*Stove (for POH i guess)*/, 
			13539/*Iron range (for POH i guess) */,
			13540/* Iron range (for POH i guess) */, 
	...
	private static final Item[] RAW_ITEMS = {
		...
		//Crayfish ??
		//new Item(3142), //Karambwan, As this thing have two cooking results, it should be hardcoded :S
		//Ugthanki Kebab
	 ...
	 */
	private static final Item[] BURNT_ITEMS = {
		...
		new Item(323), //Karambwanji //TODO: Find real burning ID for this one??
	 ...
	 **/
	private static final Item[] COOKED_ITEMS = {
	 ...
	 */
	private static final double[] EXPERIENCE_TABLE = {
		30, //Raw beef.
	...
	/**
	 * An array of the approximate levels to stop burning food.
	 * {without gauntles, with gauntles}
	 */
	//So yeah, redo all those xD.
	private static final int[][] STOP_BURNING_LEVELS = {
		{25, 20}, //Raw beef //No idea about those :(
		{25, 20}, //Raw beef - Green one from the ghost land. //No idea about those :(
		{25, 20}, //Raw rat meat //No idea about those :(
		{30, 20}, //Raw bear meat. //No idea about those :(
		{25, 20}, //Raw chicken. //No idea about those :(
		{25, 20}, //Raw chicken - Green one from the ghost land. //No idea about those :(
	 ...
	 */
	private static final Item[] ITEM_ON_ITEM_ARRAY_1 = {
	    new Item(2313), //pie dish
		new Item(2315), //pie shell for redberry pie
		new Item(2315), new Item(7172), new Item(7174), //Pie Shell, part gardening pie, part gardening pie.
		new Item(2315), new Item(7212), new Item(7214), //Pie Shell, part summer pie, part summer pie.
	 ...
	 * One of the two arrays holding the items used on each other.
	 */
	private static final Item[] ITEM_ON_ITEM_ARRAY_2 = {
		new Item(1953), //pastry dough
		new Item(1951), //redberries
		new Item(1982), new Item(1957), new Item(1965), //Tomato, onion, cabbage
		new Item(5504), new Item(5982), new Item(1955), //Strawberry, watermelon, apple
	 ...
	 */
	private static final Item[] ITEM_ON_ITEM_RESULT = {
		new Item(2315), //pie shell
		new Item(2321), //redberry pie
		new Item(7172), new Item(7174), new Item(7178), //Part gardening pie, part gardening pie, gardening pie.
		new Item(7212), new Item(7214), new Item(7216), //Part summer pie, summer pie, summer pie.
	...
	*/
	private static final int[] ITEM_ON_ITEM_LEVEL_REQUIREMENTS = {
		-1, // Just adding the pastry dough into the dish
		10, // Berry pie
	...
	/**
	 * All the mixes that shouldn't give any cooking exp, should have a -1 in here.
	 */
	private static final double[] ITEM_ON_ITEM_EXPERIENCE = {
		-1, 
		-1, 
		-1, -1, -1, //Part gardening pie, part gardening pie, gardening pie,
		-1, -1, -1 
	};
	 ...
	 * other is related to cooking.
	 * This method will also do the actual action needed.
	 * @param player The player using two items on each other.
	 * @param used The item used.
	 * @param usedWith The item the other item is being used with.
	 * @param slotUsed The slot of the item used.
	 * @param slotUsedWith The slot of the item the other item is used with.
	...
	/**
	 * Mixes the items provided, and adds the cooking experience provided, 
	 ...
	 * @param slotUsed The slot of the item used.
	 * @param slotUsedWith The slot of the item the other item is used with.
	 * @param result The item result.
	 * @param experience The experience reviecved, if value is above -1.
	 * @param sendMessage The message send for the player.
	 * @param levelReq The level requirement needed, -1 if none.
	 */
	public static final void mixCookingItems(Player player, Item used, Item usedWith, int slotUsed, int slotUsedWith, Item result, double experience, /*String sendMessage,*/ int levelReq) {
		if(levelReq != -1) {
			if(player.getSkills().getLevel(Skills.COOKING) < levelReq) {
				player.getActionSender().sendMessage("You need a Cooking level of at least "+ levelReq + " in order to mix those items.");
				return;
			}
		}
		...
		player.getActionSender().sendMessage("You mix the " + used.getDefinition().getName().toLowerCase() + " with the " + usedWith.getDefinition().getName().toLowerCase() + ".."); 
		player.getActionSender().sendMessage(".. and turn it into a " + result.getDefinition().getName().toLowerCase() + "."); //Message is getting too big :(
	...
	public static void handleInterfaceButtons(Player player, int button) {
		...
		case 5:
			...
			break;
		case 4:
			player.getInterfaceState().openEnterAmountInterface(COOKING_INTERFACE, 1 ,player.getCookingVariables().getCookingItem());
			break;
		case 3:
			cook(player, player.getInventory().getCount(player.getCookingVariables().getCookingItem()));
			break;
		}
	}
	 ...
	 */
	public static boolean checkForItemOnObject(Player player, int itemId, int itemSlot, int objectId) {
		if(sendCookingInterface(player, itemId, objectId)) {
			return true;
		}
		if(checkForWaterFillingItemAndObjects(player, itemId, itemSlot, objectId)) {
			return true;
		}
	...
	private static final Item[] FARMING_MILL_ITEMS = {
		new Item(1947), //Grain.
	};
	
	private static final int[] FARMING_MILL_OBJECTS = {
		2714, //Where you put grain in. "Grain hopper"
	};
	
	private static final Location[] FLOUR_BIN_LOCATIONS = {
		Location.create(3166, 3306, 0),
	};
	...
	private static boolean checkForFarmingMill(Player player, int itemId, int itemSlot, int objectId) {
					...
					if(object == objectId) {
						...
						case 2714://Grain hopper.
							if(player.getCookingVariables().isUsingGrainHopper()) {
								player.getActionSender().sendMessage("The hopper is already filled with grain.");
							} else {
								player.getActionSender().sendMessage("You put some grain in the hopper..");
								player.getInventory().remove(item, itemSlot);
								player.getCookingVariables().setUsingGrainHopper(true);
							}
							break;
						}
						return true;
	...
	public static boolean checkForObjectClicking(Player player, int objectId, Location loc) {
		switch(objectId) {
		/*
		...
		case 2718:
			if(player.getCookingVariables().isUsingGrainHopper()) {
				if(player.getCookingVariables().getFlourBinAmount() <= 10) {
					...
					player.getActionSender().sendMessage("The flour bin cannot contain anymore flour.");
				}
			} else {
				player.getActionSender().sendMessage("The hopper doesn't contain any grain.");
			}
			//TODO: player.getActionSender().animateObject(objx, objy, animationID, tileObjectType, orientation);
			return true;
		/*
			...
			if(player.getCookingVariables().getFlourBinAmount() > 0) {
				if(player.getInventory().contains(1931)) {//Pot
					if(player.getCookingVariables().getFlourBinAmount() == 1) {
						player.getActionSender().sendMessage("You fill a pot with the last of the flour in the bin.");
						player.getActionSender().sendCreateObject(1781, 10, 0, loc);
					} else {
						player.getActionSender().sendMessage("You fill a pot with the flour from the bin.");
					}
	 ...
	 */
	public static boolean checkForWaterFillingItemAndObjects(Player player, int itemId, int itemSlot, int objectId) {
		...
		if(checkForWaterFilling) {
			...
			for(int index = 0; index < WATER_CONTAINERS_UNFILLED.length; index++) {
				if(WATER_CONTAINERS_UNFILLED[index].getId() == itemId) {
					//player.getInventory().remove(itemSlot, WATER_CONTAINERS_UNFILLED[index]);
	...
	/**
	 * Checks if the objectId is a range, 
	 * Makes sure we have the level required,
	 * and sends the cooking interface with the 
	 * correct item id on it.
	 * @param player The player.
	 * @param itemId The item id.
	 * @param objectId The object id.
	 * @return True if the object was a range, and the item was cookable, false if not.
	 */
	private static boolean sendCookingInterface(Player player, int itemId, int objectId) {
		...
		if(player.getSkills().getLevel(Skills.COOKING) < levelToCook) {
			player.getActionSender().sendMessage("You need a Cooking level of at least " + levelToCook + " in order to cook a " + ItemDefinition.forId(itemId).getName().replace("raw", "").toLowerCase() + ".");
			return true;//We didn't successfully do what we were told, but the items existed in the array.
		}
		player.getCookingVariables().setCookingItem(itemId);
		player.getActionSender().sendInterfaceModel(307, 2, 175, itemId);
		player.getActionSender().sendString(307, 6, "<br><br><br><br>" + ItemDefinition.forId(itemId).getName().replace("", "") + "");
	...
	/**
	 * Loops through an array of cooking objects, 
	 * and checks if our objectId is in there.
	...
	/**
	 * Makes sure we don't get a null-pointer exception, 
	 * and starts the cooking event.
	 */
	public static void cook(Player player, int amount) {
		...
		int index = getItemIndex(player.getCookingVariables().getCookingItem()); //Can't be -1 anymore.
		player.getActionQueue().addAction(new CookingAction(player, index, amount));		
	}

	/**
	 * Gets the item index from the array of raw food items.
	 * @param itemId The id of the item we want an index of.
	 ...
	 * @param index The index of the item we're cooking.
	 * @param gloves The players glove, notice: this might be null.
	...
	private static int getStopBurningLevel(int index, Item gloves) {
		return STOP_BURNING_LEVELS[index][(gloves == null ? false : gloves.getId() == 775) ? 1 : 0];
	}
	
	/**
	 * The cooking event "member" class.
	 * This class is handling regular stove/fire cooking.
	 * Its getting all the needed stuff from the arrays in 
	...
	private static class CookingAction extends Action {
		 ...
		 */
		public CookingAction(Mob mob, int index, int amount) {
			super(mob, 0);
			this.rawItem = RAW_ITEMS[index];
			this.noBurnLevel = getStopBurningLevel(index, mob.getEquipment().get(Equipment.SLOT_GLOVES));
	...
	public AnimationPolicy getAnimationPolicy() {
		...
		public void execute() {
			...
			if(!getMob().getInventory().contains(rawItem.getId())){
				mob.getActionSender().sendMessage("You have run out of "+rawItem.getDefinition().getName().replaceFirst("Raw", "")+" to cook.");
				this.stop();
				//mob.getCookingVariables().setCookingItem(-1);
				return;
			}
			if(mob.getSkills().getLevel(Skills.COOKING) < levelToCook) {
				mob.getActionSender().sendMessage("You need a Cooking of level "+ levelToCook +" in order to cook this meal.");
				this.stop();
				player.getCookingVariables().setCookingItem(-1);
				return;
			}
			mob.playAnimation(Animation.create(-1));
			int mobLevel = mob.getSkills().getLevel(Skills.COOKING);
			boolean shouldBurn = (mobLevel >= noBurnLevel) ? false : shouldBurn(mobLevel, noBurnLevel);
			/*if(mob.getCookingVariables().isCookingOnFire()) {
				mob.playAnimation(FIRE_ANIMATION);
			} else {
			}*/
			mob.getInventory().remove(rawItem);
			if(shouldBurn) {
				mob.getInventory().add(burntItem);
				mob.getActionSender().sendMessage("Oops! You accidently burnt the " + rawItem.getDefinition().getName().toLowerCase() + ".");
			} else {
				mob.getInventory().add(cookedItem);
				mob.getActionSender().sendMessage("You succesfully cook the " + cookedItem.getDefinition().getName().replaceFirst("Cooked ", "").toLowerCase() + ".");//Removes the string "Cooked" from some food, eg. "Cooked meat" because it looks super idiotic.
				mob.getSkills().addExperience(Skills.COOKING, exp * Constants.EXP_MODIFIER - 150);
				try{
				Thread.sleep(3000);
				mob.playAnimation(RANGE_ANIMATION);
	 ...
	 * @param noBurnLevel The level to stop burning the food, the player is currently cooking.
