...
public class Runecrafting {
	...
	private static final Animation RUNECRAFTING_ANIMATION = Animation.create(791);
		...
		Item item = new Item(rune);
			...
			if (player.getSkills().getLevel(20) < lvlReq) {
				player.getActionSender().sendMessage(
						"You need atleast " + lvlReq + " Runecrafting to make this.");
				return;
			}
			int essenceAmount = player.getInventory().getCount(1436);
			if (!player.getInventory().contains(1436)) {
				player.getActionSender().sendMessage("You need essence to do this.");
				return;
			}
			...
			player.getActionSender().sendMessage( "You bind the temple's power into "
				...
				player.getSkills().addExperience(20, exp * essenceAmount * Constants.EXP_MODIFIER);
			int multipler = 1;

			player.getInventory().remove(new Item(1436, essenceAmount));

			if (player.getSkills().getLevel(20) >= x2 && player.getSkills().getLevel(20) < x3)
				multipler = 2;
			if (player.getSkills().getLevel(20) >= x3 && player.getSkills().getLevel(20) < x4)
				multipler = 3;
			if (player.getSkills().getLevel(20) >= x4 && player.getSkills().getLevel(20) < x5)
				multipler = 4;
			if (player.getSkills().getLevel(20) >= x5 && player.getSkills().getLevel(20) < x6)
				multipler = 5;
			if (player.getSkills().getLevel(20) >= x6 && player.getSkills().getLevel(20) < x7)
				multipler = 6;
			if (player.getSkills().getLevel(20) >= x7 && player.getSkills().getLevel(20) < x8)
				multipler = 7;
			if (player.getSkills().getLevel(20) >= x8 && player.getSkills().getLevel(20) < x9)
				multipler = 8;
			if (player.getSkills().getLevel(20) >= x9 && player.getSkills().getLevel(20) < x10)
				multipler = 9;
	...
	public static boolean portalTele(Player player, int object, int x, int y) {
		...
		//final Player player, final int rune,
			//final int lvlReq, final double exp, int x2, int x3, int x4, int x5,
			//int x6, int x7, int x8, int x9, int x10
		if (object == 2478 && x == 2843 && y == 4833) { // Air altar
			craft(player, 556, 1, 1, 11, 22, 33, 44, 55, 66, 77, 88, 99);
			return true;
		}
		if (object == 2479 && x == 2785 && y == 4840) { // Mind altar
			craft(player, 558, 2, 2, 14, 28, 42, 56, 70, 84, 98, 101, 101);
			return true;
		}
		if (object == 2480 && x == 2715 && y == 4835) { // Water altar
			craft(player, 555, 5, 2, 19, 38, 57, 76, 95, 101, 101, 101, 101);
			return true;
		}
		if (object == 2481 && x == 2657 && y == 4840) { // Earth altar
			craft(player, 557, 9, 2, 26, 52, 78, 101, 101, 101, 101, 101, 101);
			return true;
		}
		if (object == 2482 && x == 2584 && y == 4837) { // Fire altar
			craft(player, 554, 14, 2, 46, 92, 101, 101, 101, 101, 101, 101, 101);
			return true;
		}
		if (object == 2483 && x == 2524 && y == 4831) { // Body altar
			craft(player, 559, 20, 3, 46, 92, 101, 101, 101, 101, 101, 101, 101);
			return true;
		}
		if(object == 2484 && x == 2141 && y == 4832) { //Cosmic altar
			craft(player, 564, 27, 3, 59, 101, 101, 101, 101, 101, 101, 101, 101);
			return true;
		}
		if(object == 2487 && x == 2270 && y == 4841) { //Chaos
			craft(player, 562, 35, 8, 101, 101, 101, 101, 101, 101, 101, 101, 101);
			return true;
		}
		if(object == 2486 && x == 2399 && y == 4840) { //Nature
			craft(player, 561, 44, 12, 101, 101, 101, 101, 101, 101, 101, 101, 101);
			return true;
		}
		if(object == 2485 && x == 2463 && y == 4831) { //Law
			craft(player, 563, 54, 17, 101, 101, 101, 101, 101, 101, 101, 101, 101);
			return true;
		}
		if(object == 2488 && x == 2204 && y == 4835) { //Death
			craft(player, 560, 65, 25, 101, 101, 101, 101, 101, 101, 101, 101, 101);
			return true;
		}
		if(object == 17010) { //Death
			craft(player, 9075, 70, 27, 101, 101, 101, 101, 101, 101, 101, 101, 101);
			return true;
		}
		...
		if (object == 2465 && x == 2841 && y == 4828) { // Air portal
			...
			player.setTeleportTarget(Location.create(2331, 3806, 0));
			return true;
		}
		if (object == 2465 && x == 2150 && y == 3860) { // Astral portal
			player.getActionSender().sendMessage("You step through the portal...");
			player.setTeleportTarget(Location.create(2331, 3806, 0));
			return true;
		}
		...
		if(object == 2492 && ((x == 2933 && y == 4815) || (x == 2889 && y == 4813) || (x == 2885 && y == 4850) || (x == 2932 && y == 4854))) { //Rune essense exit..
			player.getActionSender().sendMessage("You step through the portal...");
			player.setTeleportTarget(Location.create(2331, 3806, 0));	
		}
		return false;
	...
	private static final Animation TELE_OTHER_ANIMATION = Animation.create(1818);
	...
	public static void teleToAbyss(final Player player, final NPC npc) {
		...
		World.getWorld().submit(new Event(1500) {
			public void execute() {
				//player.animate(1);
				player.setTeleportTarget(Location.create(abyssX[loc], abyssY[loc], 0));
				//player.getSkulls().setup(20); //Makes the skulling timer 20 minutes.
	...
	private static final Location[] RUNE_ESSENCE = {
		Location.create(2899,4818,0),
		Location.create(2910,4832,0),
		Location.create(2898,4844,0),
		Location.create(2921,4845,0),
		Location.create(2924,4818,0),
	};

	public static void teleportToRuneEssenceMiningArea(final Player player, NPC npc) {
		...
		npc.playGraphics(TELE_OTHER_GRAPHICS);
//		player.setCanWalk(false);
		npc.getUpdateFlags().flag(UpdateFlag.CHAT);
		World.getWorld().submit(new Event(1200) {

			@Override
			public void execute() {
				player.setTeleportTarget(RUNE_ESSENCE[RANDOM.nextInt(RUNE_ESSENCE.length)]);
//				player.setCanWalk(true);
				this.stop();
			}
	 ...
	 * @param objectID
	 */
	public static boolean handleAbyssObject(final Player player, final int objectID) {
		switch(objectID) {
		case 7143: //Rocks
...
//			player.playAnimation(Mining.getMiningAnimation(player));
			World.getWorld().submit(new Event(2500) {
				public void execute() {
					if(RANDOM.nextInt(4) == 0) {
						...
						player.getActionSender().sendMessage("...and you manage to break through the rock.");
						if(objectID == 7143)
							player.setTeleportTarget(Location.create(3031, 4820, 0));
						if(objectID == 7153)
							player.setTeleportTarget(Location.create(3047, 4820, 0));
					}
					//player.setCanWalk(true);
					this.stop();
				}
			...
			World.getWorld().submit(new Event(2500) {
				public void execute() {
					if(RANDOM.nextInt(4) == 0) {
						...
						if(objectID == 7147) {
							player.setTeleportTarget(Location.create(3032, 4844, 0));
						} else if(objectID == 7149) {
							player.setTeleportTarget(Location.create(3046, 4844, 0));
						}
					} else {
						player.getActionSender().sendMessage("...but you are not agile enough to get through.");
		...
		case 7150:
			int[] emotes = {
					...
					2113,2109,2111,2106,2107,2108,0x558,2105,2110,
					2112,0x84F,0x850,1131,1130,1129,1128,4275,1745,
					4280,4276,3544,3543,2836,6111,7531};
			int index = RANDOM.nextInt(emotes.length);
			//player.setCanWalk(false);
			player.getActionSender().sendMessage("You attempt to distract the eyes...");
			player.playAnimation(Animation.create(emotes[index]));
			World.getWorld().submit(new Event(2500) {
				public void execute() {
					if(RANDOM.nextInt(3) == 0) {
						player.getActionSender().sendMessage("...and you manage to sneak through.");
						if(objectID == 7146)
							player.setTeleportTarget(Location.create(3027, 4840, 0));
						if(objectID == 7150)
							player.setTeleportTarget(Location.create(3052, 4838, 0));
					} else {
						player.getActionSender().sendMessage("...but you fail to sneak through.");
					}
//					player.setCanWalk(true);
					this.stop();
				}
		...
		case 7141: //Blood rift
			player.getActionSender().sendMessage("A strange power blocks your exit");
			break;
		case 7129: //Fire rift
			teleport(player, 2574, 4849);
			break;
