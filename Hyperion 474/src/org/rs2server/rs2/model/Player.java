...
import org.rs2server.rs2.content.misc.SkillReset;
...
import org.rs2server.rs2.model.skills.CookingVariables;
...
import org.rs2server.rs2.model.skills.CraftingVariables;
...
public class Player extends Mob implements Persistable {
	...
	/**
	 * Canoe system constructor
	 */
	public Canoes getCanoes() {
		return canoes;
	}
	
	private Canoes canoes = new Canoes();
	 ...
	 * Tutor timers
	 */
	public int tutorDelay = 0;
	
	/**
	 * When true, it shows object values
	 */
	...
	public int currentSlayerTaskAmount = -1;
	public String currentSlayerTaskName = "";
	public int currentSlayerTask = -1;
...
//		super();
	...
	public void deserialize(IoBuffer buf) {
		...
		**/
		...
		this.muted = buf.getUnsigned() == 1 ? true : false;
		if (buf.hasRemaining()) {
			currentSlayerTask = buf.get();
		}
		if (buf.hasRemaining()) {
			currentSlayerTaskName = (IoBufferUtils.getRS2String(buf));
		}
		if (buf.hasRemaining()) {
			currentSlayerTaskAmount = buf.get();
		}
		if (buf.hasRemaining()) {
			tutorDelay = buf.get();
		}
	...
	public void serialize(IoBuffer buf) {
		...
		buf.put((byte) currentSlayerTask);
		IoBufferUtils.putRS2String(buf, currentSlayerTaskName);
		buf.put((byte) currentSlayerTaskAmount);		
		
		buf.put((byte) tutorDelay);
	}
