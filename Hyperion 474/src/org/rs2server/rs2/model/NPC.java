			...
			public void execute(GameEngine context) {
				...
				if(randomDrops.length > 0) {
					...
					while(drop == null) {
						...
						NPCDrop randomDrop2 = randomDrops[random.nextInt(randomDrops.length)];
						boolean alwaysDrop = randomDrop.getFrequency() >= 1;
						boolean alwaysDrop2 = randomDrop2.getFrequency() >= 1;
						if(!alwaysDrop && random.nextDouble()  < randomDrop.getFrequency()) {
							drop = randomDrop;
						}
						if(!alwaysDrop2 && random.nextDouble() / 30000 < randomDrop2.getFrequency()) {
							drop15 = randomDrop2;
						}
					}
					...
					/*
					* Since NPC's sometimes drop 2 items, we make sure 
					* that we aren't dropping the same item again.
					* (It looks awkward to drop 2 magic short bows eh?)
					*/
					if(drop15 != drop && random.nextInt(3) == 1){
					World.getWorld().createGroundItem(new GroundItem(mob.getUndefinedName(), drop15.getItem(), getLocation()), (Player) mob);
					}
				}
			} catch(Exception e) {};
