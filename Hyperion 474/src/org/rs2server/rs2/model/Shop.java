...
//		XMLController.writeXML(shops, file);
		...
		case 1: //Main stock
			if(shop.getMainStock().get(slot).getCount() > 0) {
				...
				if(!shop.hasCurrency(player, item, amount)) {
					...
					temporaryInventory.remove(new Item(shop.getCurrency(), amount * Constants.BUY_FROM_SHOP_PRICE *shop.getCostValue(player, new Item(item.getId(), amount))));
					if(!temporaryInventory.add(reward)) {
						return; //We wouldn't have enough inventory space, even after removing the currency cost.
					}					
					player.getInventory().remove(new Item(shop.getCurrency(), amount * Constants.BUY_FROM_SHOP_PRICE * shop.getCostValue(player, new Item(item.getId(), amount))));
					player.getInventory().add(reward);
	...
	public int getCostValue(Player player, Item item) {
		...
		case 1: //Main stock
			switch(getShopType()) {
			case GENERAL_STORE:
				return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.8 * Constants.SELL_TO_SHOP_PRICE < 1 ? 1 : ItemDefinition.forId(item.getId()).getValue() * 0.8 * Constants.SELL_TO_SHOP_PRICE);
		...
		case 2: //Player stock
			switch(getShopType()) {
			case GENERAL_STORE:
				return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.6  * Constants.SELL_TO_SHOP_PRICE < 1 ? 1 : ItemDefinition.forId(item.getId()).getValue() * 0.6 * Constants.SELL_TO_SHOP_PRICE);
			case SPECIALIST_STORE_BUY:
			case SPECIALIST_STORE_NO_BUY:
				return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.8  * Constants.SELL_TO_SHOP_PRICE < 1 ? 1 : ItemDefinition.forId(item.getId()).getValue() * 0.8 * Constants.SELL_TO_SHOP_PRICE);
			}
	...
	public int getSellValue(Player player, Item item) {
		switch(getShopType()) {
		case GENERAL_STORE:
			return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.4 * Constants.BUY_FROM_SHOP_PRICE < 1 ? 1 : ItemDefinition.forId(item.getId()).getValue() * 0.4 * Constants.BUY_FROM_SHOP_PRICE);
		case SPECIALIST_STORE_BUY:
		case SPECIALIST_STORE_NO_BUY:
			return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.6  * Constants.BUY_FROM_SHOP_PRICE < 1 ? 1 : ItemDefinition.forId(item.getId()).getValue() * 0.6 * Constants.BUY_FROM_SHOP_PRICE);
		}
		return 1;
	}
