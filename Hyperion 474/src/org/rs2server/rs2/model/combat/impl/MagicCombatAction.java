...
public class MagicCombatAction extends AbstractCombatAction {
	...
	if(attacker instanceof Player){
		...
		if(attacker.getCombatState().getCombatStyle() == CombatStyle.DEFENSIVE_AUTOCAST) {
			attacker.getSkills().addExperience(Skills.MAGIC, (1.33 * damage) * Constants.MAGIC_EXP_MODIFIER);			
			attacker.getSkills().addExperience(Skills.DEFENCE, (1 * damage) * Constants.MAGIC_EXP_MODIFIER);			
		} else {
			attacker.getSkills().addExperience(Skills.MAGIC, (2 * damage) * Constants.MAGIC_EXP_MODIFIER);
		}
	}
}
	...
	public static boolean canCastSpell(final Mob attacker, Mob victim, Spell spell) {
		...
		if(spell.getSpellName().contains("Tele") && !spell.getSpellName().contains("block")) {
