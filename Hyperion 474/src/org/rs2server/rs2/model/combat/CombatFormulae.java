...
public final class CombatFormulae {
	...
	public static double dragonfireReduction(Mob mob) {
		...
		if(dragonfireShield && dragonfirePotion) {
			...
			if(mob.getActionSender() != null) {
				mob.getActionSender().sendMessage("You shield absorbs most of the dragon's fire.");
			}
			return 0.90; //90%
		} else if(dragonfireShield && protectPrayer) {
			if(mob.getActionSender() != null) {
				mob.getActionSender().sendMessage("The combination of your shield and prayers fully resist the dragon's breath.");
			}
			return 1.0; //100%
