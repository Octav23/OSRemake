...
//		}
	...
	/**
	* @author 474 Server Improvement
	* This nifty little piece of code handles MODERN teleporting.
	*/
	public void initModernTele(final Location location, Graphic graphic){
		if(!canTeleport()) {
			return;
		}
		if(isBusy) {
		...
		//ActionSender.sendMessage("You cannot teleport in the wilderness.");
		return;
		}
		isBusy = true;
		playGraphics(graphic);
	...
	/**
	* @author 474 Server Improvement
	* This nifty little piece of code handles ANCIENT teleporting.
	*/
	public void initAncientTele(final Location location, Graphic graphic){
		if(!canTeleport()) {
			return;
		}
		if(isBusy) {
		...
		playAnimation(Animation.create(1979));
			World.getWorld().submit(new Tickable(5) {
				public void execute() {
					setTeleportTarget(location);
					playGraphics(Graphic.create(455, 0, 0));
					isBusy = false;
					this.stop();
				}
			});
